...
def sample(net, device, init_char, character_lookup, batch_size):
    with torch.no_grad():
        result = init_char

        input = get_input_tensor(result[-1], character_lookup)[0].to(device)
        hidden = net.init_state(batch_size)#.to(device)
